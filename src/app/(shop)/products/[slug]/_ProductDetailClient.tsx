...
export function ProductDetailClient({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/products/${slug}`)
      ...
      .then((p) => {
        ...
        setLoading(false)
        if (p.categoryId) {
          fetch(`/api/products?category=${p.category?.slug}&limit=5`)
            .then((r) => r.json())
  ...
  if (error || !product) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
      ...
      <p className="text-sm text-neutral-500">This product may have been removed or the link is incorrect.</p>
      <Link href="/products" className="rounded-full bg-neutral-900 px-6 py-2.5 text-sm font-semibold text-white hover:bg-neutral-700">
        Back to Products
      </Link>
    </div>
  )
        ...
        <div className="flex flex-col">
          ...
          <div className="mt-6 flex gap-3">
            <div className="flex items-center gap-1 rounded-full border border-neutral-300 px-3">
              ...
              <button onClick={() => setQuantity(quantity + 1)} aria-label="Increase quantity" className="p-2 text-neutral-500 hover:text-neutral-900">
              ...
              product.reviews.map((r) => (
                <div key={r.id} className="border-b border-neutral-100 pb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-neutral-900">{(r as any).user?.name ?? "Customer"}</p>
